...
/// Includes default servers for better out-of-box experience.
final mcpServerListProvider = StateProvider<List<McpServerConfig>>((ref) => [
  // Default: Filesystem server for saving medical research notes
  McpServerConfig(
    id: 'default-filesystem',
    name: 'File System (Research Notes)',
    command: 'npx',
    args: '-y @modelcontextprotocol/server-filesystem /tmp/medicortex',
    isActive: true, // Auto-connect on startup
    customEnvironment: const {},
  ),
  // Default: Brave Search for complementary web search
  // Note: Requires BRAVE_API_KEY environment variable
  // Users can add it in Settings → Edit Server → Custom Environment
  McpServerConfig(
    id: 'default-brave-search',
    name: 'Brave Search (Web)',
    command: 'npx',
    args: '-y @modelcontextprotocol/server-brave-search',
    isActive: false, // Disabled by default (needs API key)
    customEnvironment: const {
      // User needs to add: 'BRAVE_API_KEY': 'their_key_here'
    },
  ),
]);
